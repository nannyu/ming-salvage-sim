...
**条目→结果（务必过去式）**：每条记忆是「已经发生过的事」，不是待办指令。`cause` 写**哪道诏书/哪条政令**（条目），`outcome` 写**已发生的结果**。一次性政令（抄家、处死、册封、赦免、起复等）的 `outcome` 须含「已执行/已结案」意（如「魏忠贤家产已查抄入库」），**严禁**写「待推进/待见效/拟办/俟后」这类未兑现口吻——否则推演会把旧诏误当本回合还要办的事重复执行。持续性事项（清丈、剿寇、赈灾）可写在办进度，不在此限。
